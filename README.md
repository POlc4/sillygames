...
- Phase 3, palier 1 (PWA installable) : `app/manifest.ts` (servi sur `/manifest.webmanifest`), icônes PNG générées par un script Node sans dépendance, service worker `public/sw.js` écrit à la main (navigations réseau d'abord avec repli sur `/offline`, `/_next/static` cache d'abord, `/api` jamais mis en cache), enregistrement en production seulement avec bannière « Nouvelle version disponible ».
  ...
  - Trivy a bloqué l'image frontend sur une CVE critique de `tar`, dépendance de npm embarquée dans l'image de base Node. Le stage final n'a pas besoin de npm (`node server.js`) : on retire `npm`, `npx` et `corepack`, ce qui supprime la faille et allège l'image. Leçon : scanner l'image finale, pas seulement ses dépendances applicatives.
  - `actionlint` (dans un conteneur) valide les workflows avant push : il a trouvé un `run: echo "… : …"` interprété comme un mapping YAML, qui faisait échouer le workflow de déploiement au démarrage sans aucun job. Ajouté aux hooks pre-commit.
  - Le devcontainer n'avait jamais été construit : la CLI `@devcontainers/cli` (dans un conteneur Node avec le socket Docker) a révélé qu'avec deux fichiers compose, les chemins `build` du second se résolvent depuis le dossier du premier. Contexte corrigé en `.devcontainer`. Feature Node passée en v2 (PR Dependabot), image validée (1,9 Go, outillage complet).
  - Dependabot, première salve : six PR de versions majeures d'un coup. Politique retenue : les mineures et correctifs sont groupés et mergés dès que la CI passe ; les majeures qui touchent le runtime (Node), la base (Postgres, migration de données) ou la chaîne d'outils liée à Next (eslint, typescript, @types/node) sont ignorées par `dependabot.yml` et suivies à la main. commitlint acceptait mal le « Bump » capitalisé de Dependabot : règle `subject-case` désactivée.
  - La première PR de release a cassé la CI backend : release-please montait la version dans `backend/pyproject.toml`, mais `uv.lock` enregistre cette version et `uv sync --locked` refuse alors tout (tests, pip-audit, build d'image). Règle générale apprise : un outil de release ne doit jamais modifier un champ recopié par un fichier de verrouillage sans régénérer ce dernier. Désormais release-please ne met à jour que `CHANGELOG.md`, `version.txt` et son manifeste ; les versions de `pyproject.toml` et de `package.json` sont figées et documentées comme telles, et l'API annonce la version reçue au build (`APP_VERSION`).
  - Second piège de release : un tag créé par un workflow avec le `GITHUB_TOKEN` ne déclenche aucun autre workflow (protection GitHub contre les boucles). Le déclencheur `push: tags` de `deploy.yml` ne partait donc jamais et aucune image `vX.Y.Z` n'existait. Les images sont maintenant taguées quand le commit mergé est celui d'une release.
  - release-please et les PR ouvertes par les workflows exigent le réglage « Allow GitHub Actions to create and approve pull requests » (Settings > Actions > General), sinon échec explicite.
  - Piège trouvé par le scénario e2e hors ligne : recharger la page sur `controllerchange` recharge aussi à la toute première prise de contrôle du service worker, ce qui, hors ligne, affiche la page d'erreur de Chrome. On ne recharge que si l'utilisateur a cliqué « Recharger », et la nouvelle version attend (`waiting`) au lieu de s'activer d'office.
